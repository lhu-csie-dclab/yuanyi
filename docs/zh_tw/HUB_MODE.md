...
- Hub 專屬的儀表板頁面（排行榜、Peer 清單、稽核事件、拓撲），就是 client 儀表板同一個 Vue
  ...
  模式開啟後，會自動顯示「Cluster (Hub Mode)」分區。它呼叫的 JSON 端點（`/hub/api/*`）是同源的，
  由本節點透過 libp2p 代為向 Hub 索取。
...
| `server_mode.expose_gateway` | `false` | `proxy_port` 的中央 OpenAI 相容閘道是否綁在所有介面而非只有 `127.0.0.1`。Swarm 內部不會呼叫它，大多數部署都不需要開。 |
...
| 埠 | 需要被誰連到 | 被擋掉會怎樣 |
| :--- | :--- | :--- |
| `server_mode.p2p_port`（50004） | 每個節點 | 無法 bootstrap 進 Swarm。 |
| `server_mode.proxy_port`（50008） | 只需 localhost | 沒影響，見下。 |
| `web_port`（50007） | 只需維運者 | 儀表板連不上。 |

**Hub API 不需要開放任何對外埠。** `/hub/api/*` 是透過 libp2p 提供給其他節點的
（`/hub-api/1.0.0`，見 `p2p.go` 的 `HubAPIProtocolID`）。這同時讓 API 直接繼承 Swarm 本身的存取
控制：`swarm.key` 的 PSK 決定誰能加入這個私有網路，能開起那條串流的必然已經是成員——不用另外
發放金鑰，也不會有未驗證的埠暴露在外網。

`proxy_port` 還掛了另一個跟 `/hub/api/*` 無關的東西：Hub 自己的中央 OpenAI 相容閘道
（`/v1/chat/completions` 等，用的是全叢集的 P/D 視圖，不是單一節點的 gossip 已知鄰居視圖）。這
個一樣沒有 swarm 內部會呼叫——每個節點自己的 `proxy_port` 早就有等價的閘道了——所以它純粹是給
「特別想要全叢集統一派工」的外部應用程式用的選用入口。因此 `proxy_port` 預設只綁
`127.0.0.1`（`server_mode.expose_gateway: false`）；只有真的有這種外部應用時才需要設成
`true`。

瀏覽器不會說 libp2p，所以儀表板的 Hub 頁面是呼叫**自己所在節點**的 `web_port` 上的
`/hub/api/*`，由該節點透過 libp2p 去跟 Hub 拿（見 `web.go` 的步驟 4.2）。附帶好處是：任何節點的
儀表板都能看 Hub 資料，不再只有 Hub 自己。

例外是 `/hub/api/debug/*`。那些會變更叢集狀態，刻意不透過 libp2p 提供——每個節點都持有同一把
`swarm.key`，所以「swarm 成員」是遠比「有這台機器存取權的維運者」寬鬆的信任邊界。它們只在 Hub
自己的本機 HTTP 監聽埠上回應。

要確認拓樸同步是否正常，看節點日誌裡的 `[SYNC]`：健康的節點每 10 秒會記一次
`同步 Server P/D 拓樸成功`，而每一條失敗路徑現在都會記下原因。

### 重置累計統計

貢獻度計數（`total_requests`、`total_tokens`、`contribution_score`）只增不減，而 Hub 是以
keep-max 規則從 gossip 合併。所以被過去的計數錯誤污染的數值會永遠留著，除非**先從來源清起**
——順序很重要：
...
順序反過來就毫無作用：下一輪 gossip（約 3 秒）就會把節點還在廣播的舊值重新合併回來。另外要注
意 token 計數會在約 2 秒內從 vLLM 自己的 Prometheus 累計值同步回來，要真正清掉必須連 vLLM 一起
重啟；請求計數沒有這種外部來源，清了就是永久的。

Hub 儀表板本身沒有自己的 `server_mode.*` 埠來放 UI——它的頁面是同一個 Vue SPA 的一部分，跑在
client 既有的 `web_port`（預設 `50007`）上，用 hash 路由切換而不是真的另一個伺服器路徑。它呼
叫的 JSON 端點（`/hub/api/*`）則在 `server_mode.proxy_port` 上。`LoadOrCreateConfig` 會防止
