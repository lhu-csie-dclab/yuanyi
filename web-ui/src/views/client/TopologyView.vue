...
<template>
  ...
  <div class="space-y-5 sm:space-y-6 p-4 sm:p-6 lg:p-8 max-w-full min-w-0">
    ...
    <div class="card border border-blue-100 bg-gradient-to-br from-blue-50/80 via-white to-indigo-50/40 shadow-sm">
      ...
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
        <StatCard bare label="Total Tokens" :value="fmtNum(local.total_tokens)" accent class="sm:px-3" />
        <StatCard bare label="Input Tokens" :value="fmtNum(local.in_tokens)" class="pt-3 sm:pt-0 sm:px-3" />
        <StatCard bare label="Output Tokens" :value="fmtNum(local.out_tokens)" class="pt-3 sm:pt-0 sm:px-3" />
        <StatCard bare label="Requests" :value="fmtNum(local.total_requests)" class="pt-3 sm:pt-0 sm:px-3" />
        <StatCard bare label="Success" :value="fmtNum(local.success_count)" class="pt-3 sm:pt-0 sm:px-3" />
        <StatCard bare label="Uptime" :value="fmtUptime(local.uptime_seconds)" class="pt-3 sm:pt-0 sm:px-3" />
      </div>
    </div>
        ...
        <table class="w-full min-w-[640px] border-collapse text-left">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <tr v-for="(p, i) in peers" :key="p.peer_id || p.node_id || i" class="hover:bg-blue-50/30 transition-colors">
              ...
              <td class="td-cell">
                <span class="font-mono text-xs font-semibold text-brand">{{ (p.peer_id || p.node_id || '-').substring(0, 12) }}...</span>
                <span
                  v-if="(p.peer_id || p.node_id) === nodeInfo.localNodeId"
                  class="ml-1.5 rounded-md bg-brand px-1.5 py-0.5 text-[0.65rem] font-bold text-white shadow-xs"
                >ME</span>
              </td>
